...
Be careful that you do not upgrade yourself out of your own contract!  This software takes steps to prevent that from happening accidentally, but it's still possible.  One critical known way this can be achieved is by overwriting the proxy owner function selector.  If you use a simple property `address owner` as the in the proxy, it is possible for someone to make a diamond cut delegate that same exact function selector to another contract.  This would prevent you from accessing the proxy function's owner and may lock you out of your own proxy!
